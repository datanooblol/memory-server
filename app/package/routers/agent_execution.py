...
@router.post("/conversation/{convo_id}", response_model=AgentExecutionResponse)
async def create_agent_execution(
    ...
    return AgentExecutionResponse(execution_id=agent_execution_id)
...
async def get_agent_execution(
    ...
    agent_execution = await agent_execution_repo.get_by_id(execution_id)
...
async def delete_agent_execution(
    ...
    if not agent_execution_repo.get_by_id(execution_id):
        raise HTTPException(status_code=404, detail="Agent execution not found")
    await agent_execution_repo.delete(execution_id)
...
async def update_agent_execution_status(
    ...
    if not agent_execution_repo.get_by_id(execution_id):
        raise HTTPException(status_code=404, detail="Agent execution not found")
    await agent_execution_repo.patch(execution_id, dict(status=status))
    return {"message": f"Execution status updated to {status}"}

@router.patch("/{execution_id}/tasks-count")
async def update_agent_execution_tasks_count(
    ...
    await agent_execution_repo.patch(execution_id, dict(total_tasks=len(tasks)))
